...
const {regularLogin, getAuthUserInfo, getAllAccounts, getUserDetails, updateUser, deleteAccount, createNewUser} = require("../db/db-controllers/user");
...
module.exports = () => {
    ...
    router.post("/account/new", authMiddleware ,(req, res, next) => {
        return createNewUser(req.body).then((data) => {
            return res.status(200).json(data);
        }).catch(err => next(err));
